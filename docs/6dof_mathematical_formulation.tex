...
\textbf{Implementation mitigation:} To prevent numerical overflow, we clamp $\theta$ to a safe range during integration:
\begin{equation}
...
The rotational analog of Newton's second law relates the moment (torque) to angular acceleration. In body coordinates, Euler's equations for a rigid body are:
\begin{equation}
...
\textbf{Simplified form for diagonal inertia:}
Since we assume diagonal inertia, Equations~\eqref{eq:p_dot}--\eqref{eq:r_dot} further simplify to:
\begin{align}
...
\textbf{Note:} The gyroscopic coupling terms $(I_{zz} - I_{yy})qr$, etc., are often small for aircraft with similar $I_{yy}$ and $I_{zz}$, and are neglected in the simplified implementation for computational efficiency. For high-rate maneuvers or asymmetric aircraft, these terms become important.
...
For small propeller-driven aircraft, these simplifications are acceptable for control system development. More sophisticated models include thrust as a function of RPM, advance ratio, and propeller efficiency curves.
...
\begin{itemize}[noitemsep]
    ...
    \item $C_{L_\alpha}$: Lift curve slope $\frac{\partial C_L}{\partial \alpha}$. From thin airfoil theory, $C_{L_\alpha} = 2\pi$ per radian ($\approx 0.11$ per degree) for 2D flow. Finite wing effects reduce this to $C_{L_\alpha} \approx 4$--$5$ rad$^{-1}$ for typical aspect ratios.
...
Equation~\eqref{eq:CL_model} is a linearization valid for small to moderate angles of attack (typically $\alpha < 15°$). At high $\alpha$, flow separation occurs (stall), and $C_L$ decreases. A more complete model would include:
\begin{equation}
...
Our simplified model does not include stall; we clamp $\alpha \in [-30°, +30°]$ to remain in the linear regime.
...
\begin{itemize}[noitemsep]
    ...
    \item $C_{D_{\alpha^2}} \alpha^2$: Induced drag. When the wing generates lift, wingtip vortices form, creating a downwash that tilts the lift vector backward, inducing drag. From lifting-line theory:
    \begin{equation}
    C_{D,i} = \frac{C_L^2}{\pi e AR}
    \end{equation}
    where $AR$ is aspect ratio and $e$ is Oswald efficiency factor. For small $\alpha$, $C_L \approx C_{L_\alpha} \alpha$, so $C_{D,i} \propto \alpha^2$.
\end{itemize}

The $\alpha^2$ dependence captures the fact that induced drag is symmetric about $\alpha = 0$ and increases rapidly at high angles of attack.
...
\textbf{Validity:} Excellent for flights of short duration ($< 1$ hour) and limited range ($< 100$ km). The Coriolis acceleration is $\sim 2 \omega_E V \approx 0.003$ m/s$^2$ for $V = 20$ m/s, negligible compared to aerodynamic accelerations ($\sim 10$ m/s$^2$).
...
To maintain model validity, we enforce:
\begin{align}
\alpha &\in [-30°, +30°] \\
V &\geq 5 \text{ m/s}
\end{align}

These limits keep the aircraft in the linear aerodynamic regime and prevent numerical issues at very low airspeed.
...
For $M > 0.3$ (approximately $V > 100$ m/s at sea level), compressibility affects aerodynamics:
\begin{equation}
