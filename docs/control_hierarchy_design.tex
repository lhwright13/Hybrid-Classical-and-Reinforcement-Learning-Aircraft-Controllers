...
where:
\begin{itemize}[noitemsep]
    \item $p, q, r$: Roll, pitch, yaw angular rates (rad/s) in body frame
...
\begin{equation}
\phi_{cmd} = \arctan\left(\frac{V^2}{g R}\right) \approx \frac{V \dot{\psi}}{g}
\label{eq:coordinated_turn}
\end{equation}

where $R$ is turn radius and $g = 9.81$ m/s$^2$.
...
\theta_{cmd} &\approx K_{h\theta} \dot{h}_{\text{des}} \quad \text{(typically } K_{h\theta} \approx 0.1 \text{)}
\end{align}
