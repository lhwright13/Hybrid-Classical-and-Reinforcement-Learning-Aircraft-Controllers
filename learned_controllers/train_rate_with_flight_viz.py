...
def create_callbacks(config: dict, eval_env, flight_logger: Optional[FlightLogger] = None):
    """Create training callbacks including flight logging."""
    paths = config["paths"]
    ...
    # Flight logging callback (if enabled)
    if flight_logger is not None:
        # Log every 10 episodes to avoid too much data
